...
train_data = CatsDogsDataset(patient_list_txtfile='./six1/train1.txt',shuffle=True)
valid_data = CatsDogsDataset(patient_list_txtfile='./six1/test1.txt',shuffle=True)
...
# loss function
criterion = nn.CrossEntropyLoss(weight=torch.tensor([1.5,2,1,3,1,3]).to(torch.float32).cuda())
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        ...
        if (val_total_correct_number / val_sample_number)>best_accuracy:
            ...
            torch.save(model, 'ordinary_best_six_model1.pth')
    # print(
