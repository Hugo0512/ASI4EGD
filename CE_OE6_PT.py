...
train_data = CatsDogsDataset(patient_list_txtfile='./six/train1.txt',patient_list_txtfile1='./six1/train1.txt',shuffle=True)
valid_data = CatsDogsDataset(patient_list_txtfile='./six/test1.txt',patient_list_txtfile1='./six1/train1.txt',shuffle=True)
...
# loss function
criterion = nn.CrossEntropyLoss(weight=torch.tensor([1.8,11,2.5,6,1,13]).to(torch.float32).cuda())
# criterion = nn.CrossEntropyLoss(weight=torch.tensor([7.5,12,4.5,7.5,1,45]).to(torch.float32).cuda())
# criterion = nn.CrossEntropyLoss(weight=torch.tensor([10,16,6,10,1,60]).to(torch.float32).cuda())
# optimizer
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        ...
        if (val_total_correct_number / val_sample_number)>best_accuracy:
            ...
            torch.save(model, 'COE_best_six_model1.pth')
    # print(
    #     f"Epoch : {epoch+1} - loss : {epoch_loss:.4f} - acc: {epoch_accuracy:.4f} - val_loss : {epoch_val_loss:.4f} - val_acc: {epoch_val_accuracy:.4f}\n"
    # )
# torch.save(model,'six_model4.pth')
