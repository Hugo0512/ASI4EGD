...
train_data = CatsDogsDataset(patient_list_txtfile='./binary/train4.txt',patient_list_txtfile1='./binary1/train4.txt',shuffle=True)
valid_data = CatsDogsDataset(patient_list_txtfile='./binary/test4.txt',patient_list_txtfile1='./binary1/test4.txt',shuffle=True)
...
# loss function
criterion = nn.CrossEntropyLoss(weight=torch.tensor([1,4]).to(torch.float32).cuda())
# optimizer
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        ...
        if (val_total_correct_number / val_sample_number)>best_accuracy:
            ...
            torch.save(model, 'COE_best_model4.pth')
    # print(
