...
train_data = CatsDogsDataset(patient_list_txtfile='./six/train4.txt',shuffle=True)
valid_data = CatsDogsDataset(patient_list_txtfile='./six/test4.txt',shuffle=True)
...
# loss function
# weight=torch.tensor([5,8,3,5,1,30]).to(torch.float32).cuda()
criterion = nn.CrossEntropyLoss(weight=torch.tensor([2,153,3,8,1,11]).to(torch.float32).cuda())
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        ...
        if (val_total_correct_number / val_sample_number)>best_accuracy:
            ...
            torch.save(model, 'best_six_model4.pth')
    # print(
